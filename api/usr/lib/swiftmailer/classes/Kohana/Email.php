...
class Kohana_Email {
	 ...
	 */
	public static function send($to, $from, $subject, $message, $attachment = array(), $html = FALSE)
	{
        ...
        public static function notify($to, $path, $source = array(), $attachment = array()){
            
            $noreply = Kohana::$config->load('email.noreply');
            
            if(empty($noreply)){
                $noreply = 'noreply@'.$_SERVER['HTTP_HOST'];
            }
            
            $message = Kohana::message(I18n::lang().'/email', $path);
            
            if(empty($message) or !is_array($message) or !isset($message['subject']) or !isset($message['body'])){
                return FALSE;
            }
            
            if(!empty($source)){
                $message['body'] = str_replace(array_keys($source), array_values($source), $message['body']);
            }
            
            return self::send($to, $noreply, $message['subject'], $message['body'], $attachment, TRUE);
        }
